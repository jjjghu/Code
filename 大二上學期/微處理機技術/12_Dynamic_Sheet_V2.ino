...
const int buttom = 3; // 硬體中斷必須要接在 2 3 其一
#define buzzer A0
#define TFT_CS 2
#define TFT_RST 4
#define TFT_DC 7
...
volatile bool rightPlace = false; // 檢查音符是否在打擊點的變數
...
bool pressed = false; // 判斷是否有按下按鈕的變數
void flash() // 按下按鈕時觸發
{
  if(millis() - prevtime > 50) // 硬體中斷 debouncing
  {
    ...
    if(rightPlace) // 可以打且有打 => 播放音樂
    {
      rightPlace = false; // 重置變數
      pressed = true; // 更新變數
    }
  }
}
