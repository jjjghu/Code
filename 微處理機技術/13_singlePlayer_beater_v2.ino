...
void flash() // 按下按鈕時觸發
{
  if(millis() - prevtime > 10) // 硬體中斷 debouncing
  {
...
void flashB() // 按下按鈕時觸發
{
  if(millis() - prevtimeB > 10) // 硬體中斷 debouncing
  {
    //Serial.println(String("PRESSED") + ttime); //判斷 可略
...
{
  // Serial.println( String("Size Of Song:") + SongSize );
  // Serial.println(String("Size Of Tempo") + sizeof(tempo) / sizeof(int));
