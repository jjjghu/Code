...
class JobSchedular:
        ...
        while self.is_running:
            ...
            try:
                ...
                if hasattr(self, func_name):
                    ...
                    print(f"[錯誤] 無此任務: {func_name}")
                self.queue.task_done()
            except queue.Empty:
                continue

    def add_job(self, task_name: str, *args):
        self.queue.put((task_name, args))

    def start_processing(self, num_threads=3):
        if self.is_running:
            return
        self.is_running = True
    ...
    def task_add(self, x: int, y: int):
        time.sleep(1.5)
        return x + y

    def task_multiply(self, x: int, y: int):
        time.sleep(2.5)
        return x * y

    def task_power(self, x: int, y: int):
        time.sleep(3.5)
        return x ** y
...
if __name__ == "__main__":
    ...
    # demo.start()
    schedular = JobSchedular()
    schedular.start_processing()
    schedular.add_job("task_multiply", 2, 3)
    schedular.add_job("task_multiply", 5, 4)
    schedular.add_job("task_power", 2, 3)
    schedular.add_job("task_add", 1, 3)
    schedular.add_job("task_power", 3, 3)
    schedular.add_job("task_multiply", 10, 10)
    
    time.sleep(3)
    schedular.pause_processing()
    
    time.sleep(5)
    schedular.resume_processing()
    
    schedular.queue.join()
    schedular.stop_processing()
